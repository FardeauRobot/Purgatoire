...
Sed::Sed(const std::string& filepath, const std::string& replaced, const std::string& replacing)
    : m_infile(filepath), m_to_replace(replaced), m_replace_by(replacing), m_outfile(filepath + ".replace") { }

Sed::Sed(const Sed& other)
    : m_infile(other.m_infile), m_to_replace(other.m_to_replace), m_replace_by(other.m_replace_by), m_outfile(other.m_outfile) { }
