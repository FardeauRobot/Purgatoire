...
**Common errors:** `EADDRINUSE` (port in use / `TIME_WAIT` without `SO_REUSEADDR`), `EACCES` (port < 1024 needs root). Full failure table — including the silent-wrong-port and missing-`memset` traps — in [`../18_SOCKETS_AND_FDS.md`](../18_SOCKETS_AND_FDS.md#Why%20bind%28%29%20fails).
...
→ Parameters and the full CGI env-var example live with the other address-reading calls in [`04_ADDRESS_CONVERSION.md`](04_ADDRESS_CONVERSION.md#getsockname%28%29%20%E2%80%94%20find%20what%20address%20a%20socket%20is%20bound%20to).
