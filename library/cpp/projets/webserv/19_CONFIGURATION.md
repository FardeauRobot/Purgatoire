...
This file gives you a **copy-able starting config**, from the smallest one that boots up to a realistic annotated one. For *what the subject demands* of your config (the feature checklist) see [`17_WEBSERV_SUBJECT.md` §Configuration](17_WEBSERV_SUBJECT.md#Configuration%20file); for *what to get right before writing the parser* see [`TIPS.md` §Config file](TIPS.md#Config%20file%20%E2%80%94%20read%20the%20subject%20carefully). This file is the **concrete artefact** those two describe.
...
- **No `std::stoi`.** `client_max_body_size 10M` needs you to parse digits *and* the `K`/`M`/`G` suffix yourself with `std::stringstream` or manual digit accumulation. See [`cpp/io-errors/FSTREAM_GUIDE.md`](../../notions/io-errors/FSTREAM_GUIDE.md).
...
- **Print what you parsed.** Before wiring the parser to the server, dump the in-memory config tree to stdout and eyeball it against the file. If the parse is wrong, everything above it is wrong — this is the [`TIPS.md`](TIPS.md#Config%20file%20%E2%80%94%20read%20the%20subject%20carefully) advice and it's worth repeating.
...
- [`17_WEBSERV_SUBJECT.md` §Configuration](17_WEBSERV_SUBJECT.md#Configuration%20file) — the subject's required directive list (the *what*).
- [`TIPS.md` §Config file](TIPS.md#Config%20file%20%E2%80%94%20read%20the%20subject%20carefully) — pre-parser discipline and the validator habit.
