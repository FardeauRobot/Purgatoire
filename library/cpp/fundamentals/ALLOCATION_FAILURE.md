...
> **Keyword zoom-ins:** [`NEW`](../keywords/NEW.md) · [`DELETE`](../keywords/DELETE.md) · [`THROW`](../keywords/THROW.md)
...
1. [`new` throws, it never returns NULL](#1-new-throws-it-never-returns-null)
2. [Why C and C++ differ](#2-why-c-and-c-differ)
3. [No `try`/`catch` → `terminate()` → `abort()`](#3-no-trycatch--terminate--abort)
4. [`bad_alloc` abort vs segfault](#4-bad_alloc-abort-vs-segfault)
5. [Catching it, and the `nothrow` escape hatch](#5-catching-it-and-the-nothrow-escape-hatch)
6. [Simulating a failure (and the traps)](#6-simulating-a-failure-and-the-traps)
7. [Under the hood: the new-expression vs `operator new`](#7-under-the-hood-the-new-expression-vs-operator-new)
8. [Overriding `operator new`](#8-overriding-operator-new)
9. [42 guidance](#9-42-guidance)
...
a `delete`** (see [`MEMORY.md`](MEMORY.md#3-heap--new-and-delete)).
