...
- [The iterator contract every algorithm relies on](#The%20iterator%20contract%20every%20algorithm%20relies%20on)
- [The C++98 trap list (memorize this)](#The%20C%2B%2B98%20trap%20list%20%28memorize%20this%29)

**Part 1 — Container member functions**
1. [Functions common to (almost) every container](#1.%20Functions%20common%20to%20%28almost%29%20every%20container)
2. [Sequence containers — `vector` / `deque` / `list`](#2.%20Sequence%20containers%20%E2%80%94%20vector%20%2F%20deque%20%2F%20list)
3. [`std::list`'s special members](#3.%20std%3A%3Alist%27s%20special%20members)
4. [Container adapters — `stack` / `queue` / `priority_queue`](#4.%20Container%20adapters%20%E2%80%94%20stack%20%2F%20queue%20%2F%20priority_queue)
5. [Associative containers — `set` / `map` / `multiset` / `multimap`](#5.%20Associative%20containers%20%E2%80%94%20set%20%2F%20map%20%2F%20multiset%20%2F%20multimap)
6. [`std::string` as a container](#6.%20std%3A%3Astring%20as%20a%20container)

**Part 2 — The `<algorithm>` library (exhaustive, C++98)**
7. [Non-modifying sequence operations](#7.%20Non-modifying%20sequence%20operations)
8. [Modifying sequence operations](#8.%20Modifying%20sequence%20operations)
9. [Partitioning](#9.%20Partitioning)
10. [Sorting](#10.%20Sorting)
11. [Binary search (on sorted ranges)](#11.%20Binary%20search%20%28on%20sorted%20ranges%29)
12. [Merge & set operations (on sorted ranges)](#12.%20Merge%20%26%20set%20operations%20%28on%20sorted%20ranges%29)
13. [Heap operations](#13.%20Heap%20operations)
14. [Min / max](#14.%20Min%20%2F%20max)
15. [Permutations & comparison](#15.%20Permutations%20%26%20comparison)

**Part 3 — Companions**
16. [`<numeric>`](#16.%20%3Cnumeric%3E)
17. [`<functional>` — functors & adaptors](#17.%20%3Cfunctional%3E%20%E2%80%94%20functors%20%26%20adaptors)
18. [`<iterator>` — the glue](#18.%20%3Citerator%3E%20%E2%80%94%20the%20glue)
19. [Idiom cookbook](#19.%20Idiom%20cookbook)
20. [What's NOT in C++98](#20.%20What%27s%20NOT%20in%20C%2B%2B98)
...
An algorithm never knows the container — it only calls `*it`, `++it`, maybe `--it` or `it + n`. Which of those it's allowed to call is the iterator's **category** (see [`STL.md` §7](STL.md#7.%20Iterator%20categories)). The practical consequence:
...
C++98 has no lambdas. A **functor** is a class with `operator()`; you pass an *instance* where a callable is expected. The standard ships ready-made ones. (Deeper dive: [`STL.md` §10](STL.md#10.%20Functors%20%E2%80%94%20C%2B%2B98%27s%20answer%20to%20lambdas), and `operator()` itself in [`MEMBER_FUNCTION_POINTERS.md`](MEMBER_FUNCTION_POINTERS.md).)
